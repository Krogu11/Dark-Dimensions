...
export function VillageMenu({ village, message, onMarket, onRecruit, onElder, onHelp, onWaitNight, onRaid, onLeave }: VillageMenuProps) {
  ...
  const restitutionCost = gameSession.getVillageRestitutionCost(village.id);
  const week = Math.floor((gameSession.gameDay - 1) / 7) + 1;
  const access = gameSession.getVillageServiceAccess(village.id);
  const helpAvailable = Boolean(state && state.lastHelpedWeek < week && access.help);
  const mistrusted = Boolean(state && (state.relation < -10 || gameSession.currentFactionReputation < -20));
  const services = [
    { id: "market" as const, icon: "◈", title: "Village market", description: access.market ? `Buy local ${product ? t(product.nameKey) : "produce"} and simple provisions.` : "The traders refuse to deal with known raiders.", action: onMarket, disabled: !access.market },
    { id: "recruits" as const, icon: "⚔", title: "Ask for recruits", description: access.recruits ? "Hire a small number of inexpensive Tier 1 and Tier 2 volunteers." : "No villager will join a warband feared by this settlement.", action: onRecruit, disabled: !access.recruits },
    { id: "elder" as const, icon: "✦", title: state && state.relation < 0 ? "Seek atonement" : "Speak with the elder", description: !access.elder ? "The elder refuses to receive you until your standing improves." : quest?.type === "atonement" ? `Return ${quest.quantity} ${questItem ? t(questItem.nameKey) : "supplies"} to repair some of the harm, or pay restitution.` : quest?.type === "delivery" ? `The village needs ${quest.quantity} ${questItem ? t(questItem.nameKey) : "supplies"}.` : "Bandits are expected after nightfall. Wait until 22:00 and defend the village.", action: () => setFocusedService("elder"), disabled: !access.elder },
    { id: "help" as const, icon: "✚", title: "Help the villagers", description: !access.help ? "The villagers will not work beside someone they distrust." : helpAvailable ? "Spend five hours helping. Improves relations, prosperity and militia." : "You have already helped this village this week.", action: onHelp, disabled: !helpAvailable },
    ...
    { id: "raid" as const, icon: "◆", title: "Plunder the village", description: "Attack the militia and seize local stock. Markets, recruits and elders may reject you; hostile prices spread through the faction.", action: onRaid, disabled: state?.condition === "looted" },
  ];
  ...
  return <div className="city-overlay village-overlay"><main className={`city-menu village-menu focus-${focusedService}`}>
    <header className="city-header village-header"><div className="city-identity"><p className="eyebrow">Rural settlement · {state?.condition ?? "unknown"}</p><h1>{t(village.nameKey)}</h1><p>{t(village.descriptionKey)}</p></div>
      ...
      <div className="city-standing">{factionId ? <><span className={`faction-seal ${factionId}`} /><strong>{t(`faction.${factionId}.name`)}</strong><small>Village relation {state?.relation ?? 0} · Faction {gameSession.currentFactionReputation}</small>{mistrusted ? <em>Distrusted — services and prices are restricted</em> : null}</> : null}</div></header>
    <section className="city-content"><nav className="city-districts" aria-label="Village services"><span className="city-nav-label">Village life</span>{services.map((service) => <button key={service.id} className={focusedService === service.id ? "active" : ""} disabled={service.disabled} onMouseEnter={() => setFocusedService(service.id)} onFocus={() => setFocusedService(service.id)} onClick={service.action}><span className="city-service-icon">{service.icon}</span><span><small>{service.id}</small><strong>{service.title}</strong></span><b>›</b></button>)}</nav>
      <div className="city-service-focus"><span className="eyebrow">{focused.id}</span><h2>{focused.title}</h2><p>{focused.description}</p>{focused.id === "elder" && quest ? <div className="village-quest-actions"><small>{quest.type === "atonement" ? `Atonement: +${quest.rewardRelation} village relation and reduced wanted level` : `Reward: ${quest.rewardGold}g · +${quest.rewardRelation} village relation`}</small>{quest.status === "available" ? <button className="city-enter-service" onClick={() => gameSession.acceptVillageQuest(village.id)}>{quest.type === "atonement" ? "Swear restitution" : "Accept task"} <span>→</span></button> : null}{quest.type === "delivery" && quest.status === "active" ? <button className="city-enter-service" onClick={() => gameSession.completeVillageDelivery(village.id)}>Deliver {quest.quantity} {questItem ? t(questItem.nameKey) : "items"}<span>→</span></button> : null}{quest.type === "atonement" && quest.status === "active" ? <button className="city-enter-service" onClick={() => gameSession.completeVillageAtonement(village.id)}>Return {quest.quantity} {questItem ? t(questItem.nameKey) : "items"}<span>→</span></button> : null}{restitutionCost > 0 ? <button className="city-enter-service ghost" disabled={gameSession.gold < restitutionCost} onClick={() => gameSession.payVillageRestitution(village.id)}>Pay {restitutionCost} gold restitution <span>→</span></button> : null}{quest.type === "night_bandits" && quest.status === "active" ? <button className="city-enter-service" onClick={() => gameSession.isNight ? gameSession.startVillageNightDefense(village.id) : onWaitNight()}>{gameSession.isNight ? "Defend the village" : "Wait until night"}<span>→</span></button> : null}{quest.status === "completed" ? <strong>Task completed for this week.</strong> : null}</div> : <button className="city-enter-service" disabled={focused.disabled} onClick={focused.action}>{focused.disabled ? "Unavailable" : "Continue"}<span>→</span></button>}</div></section>
